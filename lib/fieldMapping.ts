...
// Type for Investor data from Prisma
type InvestorData = {
  [key: string]: any;
};
...
export interface UniversalField {
  uiField: string; // Campo no banco/UI
  pdfMappings: {
 ...
 */
export const extractUniversalData = (investor: InvestorData): Record<string, any> => {
  const data: Record<string, any> = {};

  universalFieldMappings.forEach((mapping) => {
    const value = investor[mapping.uiField];
